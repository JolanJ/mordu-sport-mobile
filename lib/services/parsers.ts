/**
 * Parsers XML pour les données Goalserve
 */
...
export function parseXMLMatches(xml: string, league: League, requestedDate?: Date): Match[] {
  try {
...
export function parseXMLScores(xml: string, league: League): Match[] {
  try {
    if (!xml || xml.trim().length === 0) {
      return []
    }
  ...
  try {
    ...
    return {
      ...
      awayTeam: {
        ...
        logo: undefined,
        score: awayScoreNum,
        teamId: awayTeam['@_id'] ? String(awayTeam['@_id']) : undefined,
      },
      homeTeam: {
        ...
        logo: undefined,
        score: homeScoreNum,
        teamId: homeTeam['@_id'] ? String(homeTeam['@_id']) : undefined,
      },
    }
  } catch (error) {
    return null
...
export function parseTeamRosterXML(xml: string): TeamRosterData | null {
  try {
    if (!xml || xml.trim().length === 0) {
      return null
    }
...
export function parseTeamStatsXML(xml: string): GoalserveTeamStats | null {
  try {
    const parsed = xmlParser.parse(xml)
...
export function parsePlayerStatsXML(xml: string): GoalservePlayerStats[] {
  try {
    const parsed = xmlParser.parse(xml)
    const statistic = parsed.statistic
...
export function parseInjuriesXML(xml: string): GoalserveInjury[] {
  try {
    const parsed = xmlParser.parse(xml)
    const team = parsed.team
...
export function parsePlayerImageXML(xml: string): string | null {
  try {
    const parsed = xmlParser.parse(xml)
...
// ============================================
// MATCH DETAILS PARSER
// ============================================

export function parseMatchDetailsXML(xml: string, matchId: string): MatchDetails | null {
  try {
    const parsed = xmlParser.parse(xml)
    let matchArray: any[] = []
